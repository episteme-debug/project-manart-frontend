$(document).ready(function name(params) {
    ...
    async function getCategoriasProductos(params) {
        try {
            ...
            var respuestaPeticion = await
            fetch('http://localhost:8080/get_categoria_producto_activa', {
                method : 'GET'
            });
    ...
    async function insertarCategoriasProductoDom() {
        ...
        if(categoriasProductos != null){
            for(var i = 0; i < categoriasProductos.length; i++){
                ...
                var tituloCard = $("<h2></h2>").addClass('h2 card-titulo').text(categoria.nombreCategoria);
                ...
                var divImagen = $("<div></div>").addClass('tiene-bg-image').css('background-image', 'url(img/' + categoria.imagenCategoria + ')');
